    ...
    f: &[i8], g: &[i8], tmp_u32: &mut [u32], tmp_fxr: &mut [FXR]) -> bool
{
    ...
    if hn >= 4 {
        ...
        let t3p: *const __m128i = transmute(t3.as_ptr());
        for i in 0..(hn >> 2) {
            let yga = _mm256_loadu_si256(t2p.wrapping_add(i));
            let ygb = _mm256_srli_epi64(yga, 32);
            let xFd = _mm_loadu_si128(t3p.wrapping_add(i));
            let yFd = _mm256_permute4x64_epi64(
                _mm256_castsi128_si256(xFd), 0x50);
            let yFd = _mm256_shuffle_epi32(yFd, 0x30);
            let yFd = mp_mmul_x4(yFd, yR2, yp, yp0i);
            let yFe0 = mp_mmul_x4(ygb, yFd, yp, yp0i);
            let yFe1 = mp_mmul_x4(yga, yFd, yp, yp0i);
            _mm256_storeu_si256(t1p.wrapping_add(i),
                _mm256_or_si256(yFe0, _mm256_slli_epi64(yFe1, 32)));
        }
    } else {
        for i in 0..hn {
    ...
    if hn >= 8 {
        ...
        let yp0i = _mm256_set1_epi32(p0i as i32);
        let yR2 = _mm256_set1_epi32(R2 as i32);
        let yq = _mm256_set1_epi32(Q as i32);
        let t1p: *const __m256i = transmute(t1.as_ptr());
        let t2p: *mut __m256i = transmute(t2.as_mut_ptr());
        let t3p: *mut __m256i = transmute(t3.as_ptr());
        for i in 0..(hn >> 3) {
            // Load values by groups of 8.
            let yf0 = _mm256_loadu_si256(t3p.wrapping_add(i));
            let yf1 = _mm256_loadu_si256(t3p.wrapping_add((n >> 3) - 1 - i));
            let yg0 = _mm256_loadu_si256(t2p.wrapping_add(i));
            let yg1 = _mm256_loadu_si256(t2p.wrapping_add((n >> 3) - 1 - i));
            let yF0 = _mm256_loadu_si256(t1p.wrapping_add(i));
            let yF1 = _mm256_loadu_si256(t1p.wrapping_add((n >> 3) - 1 - i));
            let ymg0 = mp_mmul_x8(yg0, yR2, yp, yp0i);
            let ymg1 = mp_mmul_x8(yg1, yR2, yp, yp0i);

            // Compute G.
            let yG0 = mp_div_x8(
                mp_add_x8(mp_mmul_x8(ymg0, yF0, yp, yp0i), yq, yp), yf0, yp);
            let yG1 = mp_div_x8(
                mp_add_x8(mp_mmul_x8(ymg1, yF1, yp, yp0i), yq, yp), yf1, yp);

            // We need adj(f) and adj(g), which entails reversing
            // the order of the values.
            let yfa1 = _mm256_permute4x64_epi64(yf0, 0x4E);
            let yfa1 = _mm256_shuffle_epi32(yfa1, 0x1B);
            let yfa0 = _mm256_permute4x64_epi64(yf1, 0x4E);
            let yfa0 = _mm256_shuffle_epi32(yfa0, 0x1B);
            let ymga1 = _mm256_permute4x64_epi64(ymg0, 0x4E);
            let ymga1 = _mm256_shuffle_epi32(ymga1, 0x1B);
            let ymga0 = _mm256_permute4x64_epi64(ymg1, 0x4E);
            let ymga0 = _mm256_shuffle_epi32(ymga0, 0x1B);

            let ymfa0 = mp_mmul_x8(yfa0, yR2, yp, yp0i);
            let ymfa1 = mp_mmul_x8(yfa1, yR2, yp, yp0i);
            ...
            let ykd0 = mp_add_x8(
                mp_mmul_x8(ymfa0, yf0, yp, yp0i),
                mp_mmul_x8(ymga0, yg0, yp, yp0i), yp);
            let ykd1 = _mm256_permute4x64_epi64(ykd0, 0x4E);
            let ykd1 = _mm256_shuffle_epi32(ykd1, 0x1B);

            _mm256_storeu_si256(t2p.wrapping_add(i), ykn0);
            _mm256_storeu_si256(t2p.wrapping_add((n >> 3) - 1 - i), ykn1);
            _mm256_storeu_si256(t3p.wrapping_add(i), ykd0);
            _mm256_storeu_si256(t3p.wrapping_add((n >> 3) - 1 - i), ykd1);
        }
    } else {
        for i in 0..hn {
            let tf0 = t3[i];
    ...
    if n >= 8 {
        let yp = _mm256_set1_epi32(p as i32);
        let yhp = _mm256_set1_epi32(((p + 1) >> 1) as i32);
        let t2p: *mut __m256i = transmute(t2.as_mut_ptr());
        let t3p: *mut __m256i = transmute(t3.as_mut_ptr());
        for i in 0..(n >> 3) {
            let yn = _mm256_loadu_si256(t2p.wrapping_add(i));
            let yd = _mm256_loadu_si256(t3p.wrapping_add(i));
            let yn = mp_norm_x8(yn, yp, yhp);
            let yd = mp_norm_x8(yd, yp, yhp);
            _mm256_storeu_si256(t2p.wrapping_add(i), yn);
            _mm256_storeu_si256(t3p.wrapping_add(i), yd);
        }
    } else {
        for i in 0..n {
    ...
    if n >= 8 {
        ...
        let t1p: *mut __m256i = transmute(t1.as_mut_ptr());
        let t2p: *mut __m256i = transmute(t2.as_mut_ptr());
        let t3p: *const __m256i = transmute(t3.as_ptr());
        let t4p: *const __m256i = transmute(t4.as_ptr());
        for i in 0..(n >> 3) {
                let yF = _mm256_loadu_si256(t1p.wrapping_add(i));
                let yk = _mm256_loadu_si256(t2p.wrapping_add(i));
                let yf = _mm256_loadu_si256(t3p.wrapping_add(i));
                let yg = _mm256_loadu_si256(t4p.wrapping_add(i));
                let ymf = mp_mmul_x8(yf, yR2, yp, yp0i);
                let ymg = mp_mmul_x8(yg, yR2, yp, yp0i);
                let yF = mp_sub_x8(yF, mp_mmul_x8(ymf, yk, yp, yp0i), yp);
                let yG = mp_div_x8(
                    mp_add_x8(yq, mp_mmul_x8(ymg, yF, yp, yp0i), yp), yf, yp);
                _mm256_storeu_si256(t1p.wrapping_add(i), yF);
                _mm256_storeu_si256(t2p.wrapping_add(i), yG);
        }
    } else {
        for i in 0..n {
            let tF = t1[i];
